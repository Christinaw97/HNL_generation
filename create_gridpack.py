...
def write_template(outputPath,templatePath,parameters={}):
    if parameters == {}:
        os.system(f"cp {templatePath} {outputPath}")
    else:
        f = open(templatePath)
...
def create_gridpack(
    ...
    if not args.skipClone: 
        ...
        fsparseCfg = open(os.path.join(cwdDir,'genproductions_scripts','.git','info','sparse-checkout'),'w')
    ...
    cardOutput = os.path.join(cwdDir,'genproductions_scripts','bin','MadGraph5_aMCatNLO',cardDir)
    ...
    Vmu = 0.0
    Vtau = 0.0
    if "e" in couplings.keys() and couplings['e']>1e-12:
        Ve = couplings['e']
        leptons += "e+ e- "
        neutrinos += "ve ve~ "
    if "mu" in couplings.keys() and couplings['mu']>1e-12:
        Vmu = couplings['mu']
        leptons += "mu+ mu- "
        neutrinos += "vm vm~ "
    if "tau" in couplings.keys() and couplings['tau']>1e-12:
        Vtau = couplings['tau']
        leptons += "ta+ ta- "
        neutrinos += "vt vt~ "
    ...
    proc = subprocess.Popen(
        ...
        shell = False,
        cwd = os.path.join(cwdDir,"genproductions_scripts","bin","MadGraph5_aMCatNLO"),
        #env = os.environ
    )
    proc.wait()
    ...
    shutil.move(
        os.path.join(cwdDir,"genproductions_scripts","bin","MadGraph5_aMCatNLO",cardName+"_el8_amd64_gcc10_CMSSW_12_4_8_tarball.tar.xz"),
        os.path.join(gridpackOutput,cardName+"_tarball.tar.xz")
    )
    shutil.move(
        os.path.join(cwdDir,"genproductions_scripts","bin","MadGraph5_aMCatNLO",cardName+".log"),
